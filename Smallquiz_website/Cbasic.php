...
<?php 															
																...
																$fetchqry2 = "UPDATE `cquiz` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `cquiz` where id='$c'"; 
				$result=mysqli_query($con,$fetchqry);
				$num=mysqli_num_rows($result);
				$row = mysqli_fetch_array($result,MYSQLI_ASSOC); 
			    $fetchqry2 = "SELECT * FROM `cquiz`"; 
                $result2=mysqli_query($con,$fetchqry2);

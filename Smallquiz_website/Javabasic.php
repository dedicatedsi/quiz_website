...
<?php 															
																...
																$fetchqry2 = "UPDATE `javaquiz` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `javaquiz` where id='$c'"; 
				$result=mysqli_query($con,$fetchqry);
				$num=mysqli_num_rows($result);
				$row = mysqli_fetch_array($result,MYSQLI_ASSOC); 
			    $fetchqry2 = "SELECT * FROM `javaquiz`"; 
                $result2=mysqli_query($con,$fetchqry2);
...
<form>
 <?php 
 $fetchqry1 = "SELECT * FROM `javaquiz`"; 
 $result1=mysqli_query($con,$fetchqry1);
 $num1=mysqli_num_rows($result1);
 
 if($_SESSION['clicks']>$num1){ 
	$qry3 = "SELECT `ans`, `userans` FROM `javaquiz`";
 ...
 <?php
 $qname = 'Java language basic level';

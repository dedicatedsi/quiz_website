...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `htmlquiz` where id='$c'"; 
				...
				$fetchqry2 = "SELECT * FROM `htmlquiz`"; 
                $result2=mysqli_query($con,$fetchqry2);
                $num2=mysqli_num_rows($result2);
			    $num2 = $num2+1; }
		  ?>
<tr><td><h3><br><?php echo @$row['que'];?></h3></td></tr> <?php 
  if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < $num2){ ?>
...
<form>
 <?php 
 $fetchqry1 = "SELECT * FROM `htmlquiz`"; 
 ...
 echo $no = @$_SESSION['score'];?></p>
 <?php
 $qname = 'HTML Basic level';
 ...
 $stm->close();
 $conn->close(); ?></span><br>
 <p><span>Your Score:&nbsp<?php echo "<p>$no</p>";?></span></p>
<?php } ?>
<?php
$bool = 1;
 if($bool == 1){
